...
export function ScrollTextReveal({ text, className }: ScrollTextRevealProps) {
  ...
  const { scrollYProgress } = useScroll({
    target: containerRef,
    // Start tracking when the container enters the bottom 75% of the viewport.
    // End tracking when the container reaches the top 25% of the viewport.
  ...
  return (
    <div ref={containerRef} className={cn("py-20 md:py-32 flex justify-center", className)}>
      <Typography
        variant="heading"
        className="text-4xl md:text-5xl lg:text-7xl leading-[1.1] max-w-[1200px] text-center px-4 md:px-20"
        data-cursor="text"
      >
        {words.map((word, i) => {
          ...
          // This prevents the last few words from getting stuck in a blurry state.
          const transitionLength = 0.35; // 35% of the scroll range for a smooth fade
...
function Word({ children, progress, range }: { children: string, progress: MotionValue<number>, range: [number, number] }) {
  // Map opacity from 0.15 to 1 to keep text faintly visible before reveal
